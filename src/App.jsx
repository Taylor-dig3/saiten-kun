    ...
    // <RecoilRoot>
      <BrowserRouter>
        <Routes>
          ...
          <Route path="/S1Menu" element={checkLoginState("studentLogin")?<S1Menu />:<Navigate replace to="/" />} />
          <Route path="/S2Test" element={checkLoginState("studentLogin")?<S2Test />:<Navigate replace to="/" />} />
          <Route path="/S3ResultList" element={checkLoginState("studentLogin")?<S3ResultList />:<Navigate replace to="/" />} />
          {/* <Route path="/S4TestCheck" element={checkLoginState("studentLogin")?<S4TestCheck />:<Navigate replace to="/" />} /> */}
          <Route path="/T1Menu" element={checkLoginState("teacherLogin")?<T1Menu />:<Navigate replace to="/" />} />
          <Route
            path="/T2StudentRegistration"
            element={checkLoginState("teacherLogin")?<T2StudentRegistration />:<Navigate replace to="/" />}
          />
          <Route path="/T3TestCreate" element={<T3TestCreate />} />
          <Route
            path="/T4ConfirmationCreatedTest"
            element={checkLoginState("teacherLogin")?<T4ConfirmationCreatedTest />:<Navigate replace to="/" />}
          />
          <Route path="/T5TestList" element={checkLoginState("teacherLogin")?<T5TestList />:<Navigate replace to="/" />} />
          <Route path="/T6ResultCheck" element={checkLoginState("teacherLogin")?<T6ResultCheck />:<Navigate replace to="/" />} />
        </Routes>
      </BrowserRouter>
    // </RecoilRoot>
  );
